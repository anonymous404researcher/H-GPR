function [fw,dfw] = spgp_lik_reg_nohyp(w,y,x,n,lam, del)
...
xbs = reshape(w(1:n*dim),n,dim);
reg_term = 0;
for i=1:n
    for j=1:n
        reg_term = reg_term + norm(xbs(i,:)-xbs(j,:))^2;
    end
end
reg_term = 0.5 * reg_term * lam / n;
fw = fw + reg_term;

svec = sum(xbs, 1) / n;
dfreg = lam * (xbs - repmat(svec, n, 1));

dfw = [reshape(dfxb + dfreg,n*dim,1);dfb;dfc;dfsig];
