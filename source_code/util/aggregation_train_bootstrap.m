...
M = opts.Ms;
[x_trains,y_trains,Xs,Ys] = partitionData(x_train,y_train,X,Y,M,opts.partitionCriterion,opts.covfunc) ;
...
%--------------------------------------------------------
function [xs,ys,Xs,Ys] = partitionData(x,y,X,Y,M,partitionCriterion,cov)
...
switch partitionCriterion
    ...
    case 'knkmeans'
        ...
        nr = size(x_rest,1);
        init = M-1;
        [label,~,~] = knKmeans(x_rest,M-1,cov,500,hyp_knkmeans);
        for i = 1:(M-1)
            xs{i+1} = x_rest(label==i,:) ; ys{i+1} = y_rest(label==i,:) ;
            Xs{i+1} = X_rest(label==i,:) ; Ys{i+1} = Y_rest(label==i,:) ;
        end
    case 'autoencoder'
        n_per = floor(n/M) ;
        ...
        X_rest = X(Indics,:) ; Y_rest = Y(Indics) ;
        hiddenSize = M-1;
        ae1 = trainAutoencoder(x_rest',100,...
                       'L2WeightRegularization',lr,...
                       'SparsityRegularization',sr,...
                       'SparsityProportion',sparseparam);
        feat1 = encode(ae1,x_rest');
                   
        ae2 = trainAutoencoder(feat1, 100,...
                       'L2WeightRegularization', lr, ...
                       'SparsityRegularization', sr, ...
                       'SparsityProportion', sparseparam);
        ...
        ae3 = trainAutoencoder(feat2, hiddenSize,...
                        'L2WeightRegularization', lr,...
                        'SparsityRegularization', sr,...
                        'SparsityProportion', sparseparam);
        x_rest_encoder = encode(ae3, feat2);
        
%         [~,idx]=max(x_rest_encoder,[],1);
         opts = statset('Display','off');
         [idx,C] = kmeans(x_rest_encoder',M-1,'MaxIter',1000,'Options',opts);
        for i = 1:(M-1)
            xs{i+1} = x_rest(idx==i,:) ; ys{i+1} = y_rest(idx==i,:) ;
            Xs{i+1} = X_rest(idx==i,:) ; Ys{i+1} = Y_rest(idx==i,:) ;
        end
